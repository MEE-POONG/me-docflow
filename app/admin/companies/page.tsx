...
export default function AdminCompaniesPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <p className="text-[11px] font-bold text-amber-600 uppercase tracking-wider mb-1">Company Administration</p>
        <h1 className="text-3xl font-bold text-slate-900 mb-2 font-serif">จัดการบริษัทลูกค้า</h1>
        <p className="text-sm text-slate-500">ตรวจสอบ แก้ไขข้อมูล หรืออนุมัติการยืนยันความถูกต้องให้กับบริษัทลูกค้าทั้งหมดในระบบ</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <div className="text-xs text-slate-500 font-medium">
          พบข้อมูลบริษัทในระบบทั้งหมด <span className="text-slate-800 font-bold">{filteredCompanies.length}</span> บริษัท
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-1 md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">ชื่อบริษัท</label>
                <input
                  type="text"
                  required
                  value={companyName}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">เลขผู้เสียภาษี (13 หลัก)</label>
                <input
                  type="text"
                  required
                  value={taxId}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">อีเมลเจ้าของบัญชี (Owner Email)</label>
                <input
                  type="email"
                  required
                  value={ownerEmail}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">เบอร์โทรศัพท์ติดต่อ</label>
                <input
                  type="text"
                  required
                  value={phone}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">เว็บไซต์</label>
                <input
                  type="text"
                  value={website}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">อีเมลบริษัท</label>
                <input
                  type="email"
                  required
                  value={email}
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">สถานะยืนยันตัวตนเอกสาร</label>
                <div className="flex gap-4 mt-2">
                  <label className="flex items-center text-xs text-slate-700 cursor-pointer font-sans">
                    <input
                      type="checkbox"
                      checked={isVerified}
                      onChange={(e) => setIsVerified(e.target.checked)}
                      className="h-4 w-4 text-amber-500 focus:ring-amber-500 border-gray-300 rounded mr-2"
                    />
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">ที่อยู่</label>
                <textarea
                  required
                  rows={2}
                  value={address}
              ...
              {filteredCompanies.length === 0 ? (
                ...
                filteredCompanies.map((company) => (
                  <tr key={company.id} className="hover:bg-gray-50/60 transition-colors">
                    <td className="py-4 px-6">
                      <div className="flex items-start gap-3">
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${
                          company.isVerified ? "bg-amber-50 text-amber-600 border border-amber-100" : "bg-gray-100 text-gray-400 border border-gray-200"
                        }`}>
                          <Building2 className="w-4 h-4" />
                        </div>
                        <div>
                          <p className="font-bold text-gray-900 text-xs">{company.companyName}</p>
                          <p className="text-[10px] text-slate-400 max-w-xs truncate mt-0.5" title={company.address}>
                            {company.address}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="py-4 px-6 space-y-1 text-[11px] text-slate-500">
                      <div className="text-slate-800 font-semibold truncate max-w-[180px]">
                    ...
                    <td className="py-4 px-6">
                      {company.isVerified ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-2.5 py-1 rounded-full cursor-pointer hover:bg-amber-100 transition-all font-sans"
                        ...
                        <button
                          onClick={() => handleToggleVerify(company)}
                          className="inline-flex items-center gap-1 text-[10px] font-bold text-gray-500 bg-gray-100 border border-gray-200 px-2.5 py-1 rounded-full cursor-pointer hover:border-amber-300 hover:text-amber-600 hover:bg-amber-50/30 transition-all font-sans"
