...
export default function AdminPackagesPage() {
  ...
  return (
    <div className="space-y-6 font-light">
      <div>
        <p className="text-[11px] font-bold text-amber-600 uppercase tracking-wider mb-1">Pricing Configuration</p>
        <h1 className="text-3xl font-bold text-slate-900 mb-2 font-serif">จัดการแพ็กเกจระบบ</h1>
        <p className="text-sm text-slate-500">แก้ไขข้อมูลราคา ลิมิตจำนวนผู้ใช้ พื้นที่จัดเก็บเอกสาร และรายละเอียดสิทธิพิเศษของแต่ละแพ็กเกจ</p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-xl p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5 font-sans">ชื่อแพ็กเกจแผนงาน</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5 font-sans">ราคาขายต่อเดือน (THB)</label>
                <input
                  type="number"
                  required
                  value={price}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5 font-sans">ขีดจำกัดผู้ใช้ระบบ (Max Users)</label>
                <input
                  type="text"
                  required
                  value={maxUsers}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5 font-sans">โควตาเอกสารต่อเดือน (Max Docs)</label>
                <input
                  type="text"
                  required
                  value={maxDocs}
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5 font-sans">พื้นที่คลาวด์จัดเก็บ (Max Storage)</label>
                <input
                  type="text"
                  required
                  value={maxStorage}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {packages.map((pkg) => (
          <div key={pkg.id} className="bg-white border border-gray-200 rounded-2xl p-5 flex flex-col justify-between space-y-4 hover:border-amber-300 transition-all shadow-sm">
            <div className="space-y-2">
              ...
              <h3 className="font-bold text-sm text-slate-800 pt-1">{pkg.name}</h3>
              <div className="flex items-baseline gap-1 pt-1 font-sans">
                ...
                <span className="text-xs text-slate-400 font-normal">บาท / {pkg.billing}</span>
              </div>
            </div>

            <div className="border-t border-gray-100 pt-3 space-y-2 text-xs text-slate-500">
              <div className="flex items-center gap-2">
                <Users className="w-3.5 h-3.5 text-slate-400" />
                <span>จำกัดพนักงาน: <strong className="text-slate-800">{pkg.maxUsers}</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <FileText className="w-3.5 h-3.5 text-slate-400" />
                <span>จำนวนเอกสาร/เดือน: <strong className="text-slate-800">{pkg.maxDocs}</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <HardDrive className="w-3.5 h-3.5 text-slate-400" />
                <span>พื้นที่เก็บ: <strong className="text-slate-800">{pkg.maxStorage}</strong></span>
              </div>
            </div>
          </div>
        ))}
