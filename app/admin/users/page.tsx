...
export default function AdminUsersPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <p className="text-[11px] font-bold text-amber-600 uppercase tracking-wider mb-1">User Administration</p>
        <h1 className="text-3xl font-bold text-slate-900 mb-2 font-serif">จัดการผู้ใช้งานทั้งหมด</h1>
        <p className="text-sm text-slate-500">ระงับบัญชีผู้ใช้ ปรับข้อมูลบทบาทหน้าที่ หรือรีเซ็ตรหัสผ่านของพนักงานทุกคนในเครือข่ายแอปพลิเคชัน</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <div className="text-xs text-slate-500 font-medium">
          พบพนักงานในระบบทั้งหมด <span className="text-slate-800 font-bold">{filteredUsers.length}</span> คน
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-xl p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">ชื่อ-นามสกุล</label>
                <input
                  type="text"
                  required
                  value={fullName}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">อีเมลสำหรับล็อกอิน</label>
                <input
                  type="email"
                  required
                  value={email}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">บทบาทพนักงาน (Role)</label>
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-xs text-gray-950 focus:outline-none focus:ring-2 focus:ring-amber-500 cursor-pointer"
                >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">รหัสผ่านของบัญชี</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white border border-gray-300 rounded-xl pl-3 pr-10 py-2.5 text-xs text-gray-950 focus:outline-none focus:ring-2 focus:ring-amber-500 font-mono"
                  />
                  <button
                    type="button"
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1 font-sans">สถานะบัญชี</label>
                <select
                  value={status}
                ...
                filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50/60 transition-colors">
                    <td className="py-4 px-6">
                      <div>
                        <p className="font-bold text-gray-900 text-xs">{user.fullName}</p>
                    ...
                    <td className="py-4 px-6">
                      {user.status === "active" ? (
                        <button
                          ...
                          disabled={user.id === "1"}
                          className={`inline-flex items-center gap-1.5 text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-full transition-all ${
                            user.id === "1" ? "opacity-70 cursor-not-allowed" : "cursor-pointer hover:bg-emerald-100"
                          }`}
                        ...
                        <button
                          onClick={() => handleToggleStatus(user)}
                          disabled={user.id === "1"}
                          className="inline-flex items-center gap-1.5 text-[10px] font-bold text-red-600 bg-red-50 border border-red-100 px-2.5 py-1 rounded-full cursor-pointer hover:bg-red-100 transition-all"
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg transition-colors ${
                            user.id === "1"
                              ...
                              : "text-slate-400 hover:text-red-650 hover:bg-red-55 cursor-pointer"
                          }`}
