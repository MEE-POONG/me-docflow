    ...
    <div className="space-y-6 font-light">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm transition-colors duration-200">
        ...
        <div className="text-xs text-slate-550 dark:text-slate-400 font-medium">
          พบทั้งหมด <span className="text-slate-800 dark:text-white font-bold">{filtered.length}</span> หมวดหมู่
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-xl p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200 transition-colors duration-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1.5 font-sans">ชื่อหมวดหมู่</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1.5 font-sans">รหัสหมวดหมู่ (Code - สูงสุด 4 ตัวอักษร)</label>
                <input
                  type="text"
                  required
                  maxLength={4}
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1.5 font-sans">คำอธิบายรายละเอียด</label>
                <textarea
                  value={description}
                ...
                filtered.map((cat) => (
                  <tr key={cat.id} className="hover:bg-gray-50/60 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <span className="inline-flex items-center gap-1 font-mono text-[11px] text-slate-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 px-2 py-0.5 border border-gray-200 dark:border-slate-700 rounded transition-colors duration-200">
