...
content = content.replace(
  /<aside className=\{`fixed md:sticky top-0 left-0 h-screen z-50 w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col font-sans transition-all duration-300 ease-in-out \$\{isOpen \? "translate-x-0 md:ml-0" : "-translate-x-full md:translate-x-0 md:-ml-64"\}`\}>/,
  '<aside className={`fixed md:sticky top-0 left-0 h-screen z-50 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col font-sans transition-all duration-300 ease-in-out ${isOpen ? "w-64 translate-x-0" : "w-64 md:w-20 -translate-x-full md:translate-x-0"}`}>'
);

// 2. Logo Area
const logoOrig = `      {/* Logo Area */}
...
content = content.replace(logoOrig, logoNew);
...
content = content.replace(dashOrig, dashNew);

// 4. Dropdowns
const btnRegex = /<button \s*onClick=\{\(\) => toggleMenu\('([^']+)'\)\}\s*className="w-full flex items-center justify-between px-3 py-2 (.*?)"\s*>\s*<div className="flex items-center gap-3">\s*<([A-Za-z0-9_]+) size=\{20\} className="text-gray-400" \/>\s*<span className="text-sm font-medium">(\{t\.sidebar\.[^}]+\})<\/span>\s*<\/div>\s*\{expanded\.[^ ]+ \? <ChevronDown size=\{14\} className="text-gray-400" \/> : <ChevronRight size=\{14\} className="text-gray-400" \/>\}\s*<\/button>/g;

content = content.replace(btnRegex, (match, menuKey, classes, icon, textVar) => {
  return `<button 
              onClick={() => toggleMenu('${menuKey}')}
              className={\`w-full flex items-center \${isOpen ? 'justify-between px-3' : 'justify-center px-0'} py-2 ${classes}\`}
              title={!isOpen ? ${textVar} : undefined}
            >
              <div className={\`flex items-center \${isOpen ? 'gap-3' : 'justify-center'}\`}>
                <${icon} size={20} className="text-gray-400 shrink-0" />
                {isOpen && <span className="text-sm font-medium truncate">{${textVar}}</span>}
              </div>
              {isOpen && (
                expanded.${menuKey} ? <ChevronDown size={14} className="text-gray-400 shrink-0" /> : <ChevronRight size={14} className="text-gray-400 shrink-0" />
              )}
            </button>`;
});

// 5. Approval system
const appOrig = `<button 
...
content = content.replace(appOrig, appNew);

// 6. Sub-menus
content = content.replace(/\{expanded\.([A-Za-z0-9_]+) && \(/g, '{isOpen && expanded.$1 && (');
...
content = content.replace(repOrig, repNew);
...
content = content.replace(pwdOrig, pwdNew);

// 9. Logout
const logOrig = `<button 
...
content = content.replace(logOrig, logNew);

// 10. Footer
const footOrig = `      {/* Footer hint */}
...
content = content.replace(footOrig, footNew);
